...
function getProductosPorSubcategoria(subcategoriaId) {
  return PRODUCTOS.filter(p => p.subcategoria === subcategoriaId)
}
...
function getBlogEntry(slug) {
  return BLOG.find(e => e.slug === slug)
}
...
function getProductosPorMarca(marcaId) {
  return PRODUCTOS.filter(p => p.marca === marcaId)
}

function getProductosPorTag(tagId) {
  return PRODUCTOS.filter(p => p.tags.includes(tagId))
}

function getBlogPorTag(tagId) {
  return BLOG.filter(e => e.tags.includes(tagId))
}
